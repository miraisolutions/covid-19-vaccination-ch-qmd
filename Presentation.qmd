...
- Quarto has an easier way to handle configurations from `yml` file, standardized and flexible w.r.t. layout and output format.

- Quarto has already some additional features, Tab panels, code highlighting, notations, freeze results etc.
...
```
Without #| context: data, the code will fail to find `data_old_faith`, the `#| context: server` opens a new R process that does not know what was executed before.
