...
- Quarto has an easier way to handle configurations from `YAML` file, standardized and flexible w.r.t. layout and output format

- Quarto has already some additional features: tab panels, code highlighting, notations, freeze results etc. It has many [templates available](https://github.com/mcanouil/awesome-quarto#templates){target="_blank"}.
...
- `yml` files can be configured at project level (`_quarto.yml`) or for single documents, options are available where R code was required 
...
- It facilitates the use of content across multiple documents, easier to handle than [R Markdown child-documents](https://bookdown.org/yihui/rmarkdown-cookbook/child-document.html){target="_blank"}.
...
Every Tuesday, through the GitHub Actions [workflow]((https://github.com/miraisolutions/covid19-vaccination-ch/blob/main/.github/workflows/workflow.yml){target="_blank"}), the app fetched data from BAG and re-built the datasets stored as `.rds` in the `inst` folder.
...
[https://github.com/miraisolutions/covid-19-vaccination-ch-qmd-pkg](https://github.com/miraisolutions/covid-19-vaccination-ch-qmd-pkg){target="_blank"}
...
Few modifications are required in the `worflow.yml`, making use of [quarto actions](https://github.com/quarto-dev/quarto-actions){target="_blank"}

Prior to deploying to shinyapps.io Quarto must be installed.
```
- name: Install Quarto
...
- Update in deploy script, deployment with `quarto_publish_app`
- Care required for the shiny content
- shinyapps.io the only solution for publishing interactive documents

The [shiny app](https://miraisolutions.shinyapps.io/covid-19-vaccination-ch-qmd-pkg/){target="_blank"} on shinyapps.io. A table of contents appears.
...
- Files to be sourced
- Roxygen documentation missing
- `renv` allowed
- missing `DESCRIPTION` file
- tests can be placed in a second `Tests.qmd` file to be rendered also in CI

See the app on [shinyapps.io](https://miraisolutions.shinyapps.io/covid-19-vaccination-ch-qmd/){target="_blank"}

# Final Remarks

## Is it worth moving from Rmd to Qmd?

It is definitely worth starting new projects and documents using Quarto, for the reasons mentioned above.
...
Quarto allows however using more features in terms of layout, annotations, etc. therefore if you plan to develop an app farther and make it more user-friendly, Quarto can give more options.

## References

- [Mirai Solutions Technical Guidesl](https://mirai-solutions.ch/techguides/){target="_blank"} on `Renv`, GitHub Actions CI-CD, roxygen2.
- Example Shiny package with CI-CD on Mirai Repo:
  https://github.com/miraisolutions/ShinyCICD
- [Quarto Interactivity - Shiny](https://quarto.org/docs/interactive/shiny/){target="_blank"}
