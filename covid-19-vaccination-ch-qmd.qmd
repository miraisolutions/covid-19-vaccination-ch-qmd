...
The current situation of the last 4 weeks as of **`r weeks_from_to[2]`**, i.e. how the infections, hospitalizations and deaths occurred across the age classes in absolute terms is shown below. Overall during this period Switzerland has registered [**`r .extract_value(datacases, "Infections")`**]{style="background-color: #dfc77f"} infections, [**`r .extract_value(datacases, "Hospitalizations")`**]{style="background-color: #dfc77f"} hospitalizations and [**`r .extract_value(datacases, "Deaths")`**]{style="background-color: #dfc77f"} deaths.
...
```{r }
#| label: fig-cases
#| fig-cap: "Positive, hospitalized and dead cases in the last 4 weeks."
#| warning: false
#| fig-alt: "Positive, hospitalized and dead cases in the last 4 weeks."
...
```{r}
#| label: fig-cases-100k
#| fig-cap: "Positive, hospitalized and dead cases in the last 4 weeks over 100k."
#| warning: false
#| fig-alt: "Positive, hospitalized and dead cases in the last 4 weeks over 100k."
...
```{r }
#| label: fig-vac-status
#| fig-cap: "Current vaccination status."
#| warning: false
#| fig-alt: "Current vaccination status."
plotcol <- pick_vac_lev_col(c("partially_vaccinated", "fully_vaccinated_no_booster", "fully_vaccinated_first_booster"))
plotly_out <- StackedBarplotCovid(datavacc, g_palette = c(plotcol), X = "AgeClass", FILL = "Status", 
...
<!-- # ```{r } -->
<!-- # browser() -->
<!-- # htmltools::includeHTML(plotly_out) -->
<!-- #  -->
<!-- # ``` -->

[**`r datavacall %>% filter(Status %in% names(vac_levels())[vac_levels() == "fully_vaccinated_no_booster"]) %>% .[,"Value"]`**]{style="background-color: #dfc77f"} of the Swiss population is fully vaccinated (2 doses), [**`r datavacall %>% filter(Status %in% names(vac_levels())[vac_levels() == "partially_vaccinated"]) %>% .[,"Value"]`**]{style="background-color: #dfc77f"} has received at least one dose, while [**`r datavacall %>% filter(Status %in% names(vac_levels())[vac_levels() == "fully_vaccinated_first_booster"]) %>% .[,"Value"]`**]{style="background-color: #dfc77f"} has received the booster dose.
...
```{r}
#| label: fig-vac-breakthrough
#| fig-cap: "Last 4 weeks vaccination breakthroughs."
#| warning: false
#| fig-alt: "Last 4 weeks vaccination breakthroughs."
...
```{r}
#| label: fig-vac-breakthrough-100K
#| fig-cap: "Last 4 weeks vaccination breakthroughs Vac vs Unvac over 100k."
#| warning: false
#| fig-alt: "Last 4 weeks vaccination breakthroughs Vac vs Unvac over 100k."
#| 
# scale to 100k
...
```{r}
#| label: fig-vac-hosp-100K
#| fig-cap: "Last 4 weeks Hosp. Deaths Vac vs Unvac over 100k."
#| warning: false
#| fig-alt: "Last 4 weeks Hosp. Deaths Vac vs Unvac over 100k."
...
The estimate indicate that the "Unvaccinated" people have [**`r DataPlot100kScaleRatios %>% filter(AgeClass == "All" & Status == pick_vac_lev("fully_vaccinated_first_booster") & grepl("Ratio", Variable) & Case == "Hospitalizations") %>% as.data.frame() %>% .[,"value"]`**]{style="background-color: #dfc77f"} times higher risk to be hospitalized, [**`r DataPlot100kScaleRatios %>% filter(AgeClass == "All" & Status == pick_vac_lev("fully_vaccinated_first_booster") & grepl("Ratio", Variable) & Case == "Deaths") %>% as.data.frame() %>% .[,"value"]`**]{style="background-color: #dfc77f"} times higher risk to die compared with a "Fully Vaccinated with Booster", while [**`r DataPlot100kScaleRatios %>% filter(AgeClass == "All" & Status == pick_vac_lev("fully_vaccinated_no_booster") & grepl("Ratio", Variable) & Case == "Hospitalizations") %>% as.data.frame() %>% .[,"value"]`**]{style="background-color: #dfc77f"} and [**`r DataPlot100kScaleRatios %>% filter(AgeClass == "All" & Status == pick_vac_lev("fully_vaccinated_no_booster") & grepl("Ratio", Variable) & Case == "Deaths") %>% as.data.frame() %>% .[,"value"]`**]{style="background-color: #dfc77f"} times higher compared with "Fully Vaccinated without Booster".
...
These opposite scenarios can be generated and compared with what really happened in the last 4 weeks ("Curre
nt") by taking the *Hospitalization* and *Death* rates over 100'000 people of the unvaccinated ("No Vac.") and vaccinated ("Vac. Booster" / "Vac No Booster") populations and projecting them over the full population.
...
The Incidence over 100'000 people in the 3 scenarios + "Current" are presented below, and more importantly, projecting the values of the 3 scenarios on the whole population we can evaluate the vaccination impact in absolute terms.

::: {.panel-tabset}

### Over 100 K
```{r}
#| label: fig-vac-hosp-scen-100K
#| fig-cap: "Last 4 weeks Hosp. Deaths Vac vs Unvac Scenarios over 100k."
#| warning: false
#| fig-alt: "Last 4 weeks Hosp. Deaths Vac vs Unvac Scenarios over 100k."
...
```{r}
#| label: fig-vac-hosp-scen
#| fig-cap: "Last 4 weeks Hosp. Deaths Vac vs Unvac Scenarios projections."
#| warning: false
#| fig-alt: "Last 4 weeks Hosp. Deaths Vac vs Unvac Scenarios projections."
...
If there had been no vaccination at all, in the last 4 weeks there would have been [`r DataTabp %>% filter(AgeClass == "All" & Status == "No Vac." & Case == "Hospitalizations") %>% as.data.frame() %>% .[,"value"] %>% .funformat(FALSE)`]{style="background-color: #dfc77f"} *Hospitalizations* and [`r DataTabp %>% filter(AgeClass == "All" & Status == "No Vac." & Case == "Deaths") %>% .[,"value"] %>% .funformat(FALSE)`]{style="background-color: #dfc77f"} *Deaths*, on the contrary, if all had received booster, there would have been [`r DataTabp %>% filter(AgeClass == "All" & Status == "Vac. Booster" & Case == "Hospitalizations") %>% .[,"value"] %>% .funformat(FALSE)`]{style="background-color: #dfc77f"} *Hospitalizations* and [`r DataTabp %>% filter(AgeClass == "All" & Status == "Vac. Booster" & Case == "Deaths") %>% .[,"value"] %>% .funformat(FALSE)`]{style="background-color: #dfc77f"} *Deaths*.
...
```{r }
#| context: server
#| label: fig-vac-hosp-impact-age
#| fig-cap: "Last 4 weeks Impact Hosp. Deaths Vac vs Unvac per age class."
#| warning: false
#| fig-alt: "Last 4 weeks Impact Hosp. Deaths Vac vs Unvac per age class."

output$lineplot1 <- plotly::renderPlotly({
...
plotly::plotlyOutput("lineplot2")
```

```{r lineplot2_output_plot}
#| context: server 
#| label: fig-vac-hosp-impact-ratio-age
#| fig-cap: "Last 4 weeks Ratio Hosp. Deaths Vac vs Unvac per age class."
#| warning: false
#| fig-alt: "Last 4 weeks Ratio Hosp. Deaths Vac vs Unvac per age class."

output$lineplot2 <- plotly::renderPlotly({
...
This page article aims also at showing how an **Quarto** file can be deployed to **shinyapps.io** to be a live page. For details on the architecture of the **covid19vaccinationch** R package that supports this deployment and its installation see the [README](https://github.com/miraisolutions/covid19-vaccination-ch#readme){target="_blank"} on our public **GitHub** repository or read the related [**news post**](https://mirai-solutions.ch/news/2022/03/08/announce-covid19-vaccination-ch/){target="_blank"} where we described it more in details.
